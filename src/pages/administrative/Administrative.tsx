...
import {
  ...
  getSearchData,
} from '../../reduxStore/reducer/administrativeReducer';
...
export interface ExtendedICellRendererParams extends ICellRendererParams {
  setIsEdit: any;
  handleUpdate(data: Administrative): void;
...
function Administrative() {
  ...
  const [initialValues, setInitialValue] = useState<Administrative>({
    id: '',
    organization_name: '',
    name: '',
    short_name: '',
  ...
  const handleUpdate = (data: Administrative) => {
    setOpen(true);
    setInitialValue(() => {
  ...
  const getData = (data: any, params: any) => {
    dispatch(getAdministrativeData(data, params));
  };
  ...
  const handleDelete = (data: Administrative) => {
    dispatch(deleteAdministrativeData(data, gridApi));
  };
  ...
  const buttonProps = {
    children: ' Add Administrative Office',
  ...
  return (
    <main>
      <header className="header">Administrative Office</header>
      <GridHeader2 textFieldProps={textFieldProps} buttonProps={buttonProps} />

      <ServerSideGrid
        rowDataUrl="/administrative/"
        debouncedSearchQuery={searchQuery}
        columnDefs={administrativeDefs}
        gridApi={gridApi}
        columnApi={columnApi}
      ...
      <Dialog open={open}>
        ...
        <Formik
          ...
          onSubmit={(values) => {
            if (!isEdit)
              dispatch(addAdministrativeData(values, setOpen, '', gridApi));
            else {
              dispatch(editAdministrativeData(values, setOpen, gridApi));
              handleClickClose();
            }
          }}
        >
            ...
            >
              <Form>
                ...
                <Field
                  ...
                  label="Administrative Office Name"
                  variant="standard"
                  name="name"
                  value={values.name}
